...
- 当前改动仍未上传体验版 / 提审 / 发布。
- 本轮改动将随当前功能分支提交并推送；正式发布前需确认合并到目标发布分支。
...
- 首页 / 日历「开饭雷达」文案按时段和库存状态变化：
  ...
  - 按稳定、临期、过期状态生成「状态稳定 / 变黄 / 变红」提示。
- 首页雷达和日历雷达补充「已处理」按钮：
  - 用于把已吃掉或已清理的风险食材从库存移除。
  - 操作前仍有二次确认，避免误删。
...
- 2026-06-17 v1.2 收口改动正在 `feat/home-radar-merge` 功能分支整理
...
- 首页「开饭雷达」支持一键标记风险食材为已处理，确认后从库存移除
- 食材缩略图按品类 / 食材族群显示写实图
...
- 雷达运行快检确认按时段和临期状态输出 `statusText`、`timeText`、`actionItems` 和 `processText`。
- `git diff --check` 通过。
...
- 添加页新增「最近常加 / 再加一次」。
- 首页「开饭雷达」决策提示。
- 首页 / 日历「开饭雷达」新增按时段 / 状态变化的文案和「已处理」入口。
- 空冰箱样板食材批量建库。
- 订阅消息授权灰度入口。
