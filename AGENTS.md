...
- 日历页显示到期食品
- 菜谱页基于临期食品返回本地规则推荐
- 拍照录入和扫码录入第一阶段走 mock 解析，并必须经过确认页后保存
...
- 微信小程序项目结构
- CloudBase 初始化
- CloudBase 数据库集合：`items`、`reminders`、`parseLogs`
...
- `services/recipeService.js`：本地规则菜谱推荐
