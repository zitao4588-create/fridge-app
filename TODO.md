...
当前日历页新增“临期去化”模块，菜谱页已固定为库存感知的 AI 菜谱体验页。AI / OCR 能力已在云函数侧接入真实调用路径，但仍保留 mock 兜底；拍食品已接入智谱视觉识别，拍包装和小票走腾讯 OCR + Hunyuan，条形码扫描模块已从用户入口移除，真实天气仍未接入。
...
- 在 CloudBase 控制台为 `recipeRecords` 增加组合索引：
  - `_openid` 升序、`createdAt` 降序，用于收藏列表排序。
  - `recipeKey` 升序、`_openid` 升序，用于收藏去重查询。
