...
项目当前主线是微信小程序“冰箱小雷达”，不是旧的 React/Vite H5。
...
## 2026-05-30 下一版本图片治理方向

已记录：

- 新增可持续维护的版本指导文档：
  - `NEXT_VERSION_GUIDE.md`
- 下一版本优先建议做“上线后稳定性 + 图片资源治理”。
- 菜谱图不要按每道菜单独维护，改为按标签共用图。
- 当前建议的菜谱通用图标签：
  - 清蒸类
  - 炖汤类
  - 快炒类
...
- 判断优先级：
  ...
  - 做法不明显时按主食材族群匹配。
  - 都判断不出来时使用通用健康家常菜图。
- 图片云存储化应小步做，先从最大、最不适合放主包的菜谱 / 食材图开始。
- 云存储清理分两类：
  - 删除食品时清理该食品绑定的单个 `imageFileId`。
  - 孤儿图片先做扫描报告，不直接删除。

优先级高：

- 下一轮开始前先读 `NEXT_VERSION_GUIDE.md`。
- 先盘点 `images/` 目录体积和微信预览主包大小。
- 优先调整 `utils/visualAssets.js`，让菜谱图按标签共用，而不是按具体菜名无限扩展。

注意事项：

- 不要继续无控制增加本地高清 PNG。
- 不要一次性批量删除本地图片或云存储文件。
- 图片治理不要混入登录、支付、独立后端、Docker 等复杂能力。

## 2026-05-31 宣传物料独立项目待办

宣传视频和营销物料已迁移到同级独立项目：

- `../fridge-radar-promo`

本小程序仓库后续只保留产品真实功能、技术状态和宣传边界，不再维护视频工程源码。

后续小程序侧需要配合宣传物料的事项：

- 每次新增或调整真实功能后，及时更新 `PROJECT_CONTEXT.md`。
- 每次改变产品定位、功能边界或禁止宣传项后，及时更新 `DECISIONS.md`。
- 提供真实小程序截图时，优先按宣传项目的 `ASSET_REQUESTS.md` 替换素材。
- 宣传物料更新前，确认内容仍只表达真实功能：记录食材、设置保质期、临期提醒、库存做饭思路、反馈和手动修改。

宣传项目侧待办移至：

- `../fridge-radar-promo/PROMO_CONTEXT.md`
- `../fridge-radar-promo/ASSET_REQUESTS.md`
