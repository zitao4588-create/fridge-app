...
完成等级：C3 已完成正式后端切换和 `2.0.0` 代码包上传；双账号核心链路和餐盘真机手势已通过。用户决定跳过成员移除后的越权验证，该风险保留但不再阻断提审。微信公众平台尚未提交审核或发布。
...
- `familyInventory` 已移除测试前缀并部署安全修正版；未操作生产数据迁移。`2.0.0` 已上传，尚未在微信公众平台提交审核或发布。
...
- Git 提审提交 `2a6f6e4` 已推送到 `origin/main`。
- v2.0 官网已同步上线，核心页面均返回 200 并显示当前版本口径。
- `sendExpiryReminders` 线上已切换到每日汇总和授权余额逻辑，定时触发器仍为每天 09:00。
- `generateRecipes`、`parseFoodImage`、`parseBarcode` 已部署默认关闭总闸门，安全调用确认能力未开放。
- 微信开发者工具上传 `2.0.0` 成功：总包 720.4 KB，主包 437.5 KB，`pkg-add` 分包 282.9 KB。
- 微信公众平台的隐私指引、v2.0 知识库和提交审核仍需人工完成；审核通过后的正式发布不在本轮授权范围。
