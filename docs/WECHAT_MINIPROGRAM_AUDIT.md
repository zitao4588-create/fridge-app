...
| `sendExpiryReminders` | 已部署 | 线上已包含每日汇总、授权余额和业务结果统计；定时触发器仍为每天 09:00，待双账号验证完整授权消耗 |
| `parseFoodImage` | 已部署且默认关闭 | v2.0 前端不开放；安全调用返回 `FEATURE_DISABLED`，未下载图片或执行识别 |
| `parseBarcode` | 已部署且默认关闭 | v2.0 前端不开放；安全调用返回 `FEATURE_DISABLED`，未执行搜索或识别 |
| `generateRecipes` | 已部署且默认关闭 | v2.0 前端不开放；安全调用返回能力未开放，未调用 AI 模型 |
...
- [x] 定时触发器配置为每天 09:00。
- [x] 部署后确认线上包使用 `expirySummary` 记录同一用户同一天的汇总发送状态，避免重复提醒。
...
- [x] 部署后核对线上函数包，确认已包含每日汇总、授权余额和结构化业务统计逻辑。
...
- [x] 部署后确认三个新总闸门均未开启，安全调用未触发 AI、图片下载或识别。
...
- [ ] 在微信公众平台后台重新上传 v2.0 新版知识库，并确认状态为「已发布」。
- [ ] 后台知识库文件名、发布时间和本地源文件版本保持可追溯。
- [ ] v2.0 新版发布后再单独确认是否删除 v1.2 知识库；删除属于云端资源清理，不在本次自动操作范围。
...
- [x] 本轮微信开发者工具预览包体积。
- [x] 真机预览完整流程。
- [x] 微信开发者工具上传版本 `2.0.0`，总包 720.4 KB。
- [ ] 微信公众平台对 `2.0.0` 提交审核。
- [ ] 提审截图和功能说明。
...
v2.0 正式后端和安全闸门已经部署，版本 `2.0.0` 已上传；微信公众平台尚未提交审核或发布。完成后台隐私指引、知识库和审核说明后可提交审核；审核通过不等于已正式发布。
